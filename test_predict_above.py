...
def test_transits_above():
    ...
    transits = predict.transits(tle, qth, ending_after=ENDING_AFTER, ending_before=ENDING_BEFORE)
    ...
    for i, t in enumerate(TRANSITS_REF):
        if transits[i]["start"] < t["start"] - TOLERANCE or t["start"] + TOLERANCE < transits[i]["start"]:
            test_ok = False
        if transits[i]["end"] < t["end"] - TOLERANCE or t["end"] + TOLERANCE < transits[i]["end"]:
            test_ok = False
